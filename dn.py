...
class background_tasked(commands.Cog):
	...
	def __init__(self, bot):
		...
		self.bot = bot
		self.link = "http://sea.dragonnest.com/news/notice/all"
	...
	async def feedtask(self):
		...
		ch = self.bot.get_channel(696558477698400347)#currently DNCD exclusive
		if ch is None:
			return
					...
					async with session.get(self.link) as r:
						if r.status == 200:
							...
							if int(currentnum) > current or (tit.lower() != currenttit and int(currentnum) >= current):
								...
								await ch.send("<@&688908192599507010>",embed=embed)
